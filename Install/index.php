    ...
    if ($step == 5)
    {
        ...
        if ($setupsta == 0)
        {
            ...
            */
            ...
            $errstr = GetBackAlert("写入配置 /common.php 失败，请检查 /config.inc.php 文件夹是否可读写！");
            $fp = fopen(ROOTDIR."/common.php", "w+") or die($errstr);
            flock($fp, 2);
            fwrite($fp, $strConfig);
            fclose($fp);
            ...
            while (!feof($fp))
            {
                ...
                {
                    $query .= $line;
                    $query = str_replace('review_', $dbPrefix, $query);
        ...
        {            
            ...
            $setupinfo = "
            	        已完成所有项目的安装。<br />
            	        现在转入(<a href='{$gotourl}'>主页</a>)，请稍候……<br />
            	        如果系统太长时间没有反应，请点击<a href='{$gotourl}'>这里</a>。
            	      ";
            include_once ("./templates/s4.html");
            exit ();
